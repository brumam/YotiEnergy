...
1) [Running the profile examples](#running-Yoti-Energy) -
...
2. Rename the [.env.example](yotienergy/.env.example) file to `.env` and fill in the required configuration values (mentioned in the [Configuration](#configuration) section)
3. Retrieve dependencies with `go get`
4. Start the server with `go run main.go certificatehelper.go`
